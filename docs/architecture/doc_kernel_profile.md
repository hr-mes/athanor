...
Status: **draft for maintainer review, revision 13 (2026-09-14).** Revision 2 folded in a
second full audit of the platform; revisions 3 to 13 fold in approval-gate verification
...
`rd.*`, `systemd.*`, `init=`, `efi=`, `iommu=`, `iommu.passthrough=`, `intel_iommu=`, `amd_iommu=`,
...
an addon alone, except the role selection itself (`athanor.role=`), whose removal is
visible only to attestation through the PCR 12 event log (section 10, residual risks).
...
1. **CI, no VM:** the `profile.toml` validator over every role combination;
   ...
   set it admits; interactive plus mesh and mesh plus `user-modules` are rejected); every class policy of the release is compared with every policy of a different class
...
5. **Release 1.0 updates** (P4b, in a VM with Secure Boot and swtpm): an update with an
   ...
   unannounced MokList enrolment; a reseal started from the compatibility profile is
   ...
   menu and asks for the recovery key or passphrase (D42).
